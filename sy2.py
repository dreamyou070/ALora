...
gquery_transformer = GlobalQueryTransformer(hidden_dim=160,
                                            num_feature_levels=4,
                                            with_fea2d_pos=True)
query_list = [torch.rand(8,64,160),
              torch.rand(8,256,160),
              torch.rand(8,1024,80),
              torch.rand(8,4096,40)]
out = gquery_transformer(query_list)
pi, pisi = out
print(f'pi : {pi.shape}')
print(f'pisi : {pisi.shape}')