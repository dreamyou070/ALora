...
def main(args):
    ...
    vit_model.to(accelerator.device, dtype=weight_dtype)
    vit_model.eval()
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.do_normal_sample:
                with torch.no_grad():
                    ...
                    global_feat = vit_model(batch["image"].to(device, dtype=weight_dtype)).last_hidden_state
                anomal_position_vector = torch.zeros_like(object_position_vector)
                model_kwargs = {}
                model_kwargs['global_feat'] = global_feat
                with torch.set_grad_enabled(True):
